  ...
  $.widget("mage.configurable", {
    ...
    _initializeOptions: function () {
      var options = this.options,
        ...
        priceBoxOptions =
          $(this.options.priceHolderSelector).priceBox("option").priceConfig ||
          null;
      ...
      options.parentImage = $("[data-role=base-image-container] img").attr(
        "src"
      );
        ...
        this.options.settings.each(
          $.proxy(function (index, element) {
            ...
            if (typeof this.options.productUrls !== "undefined") {
              $.each(
                this.options.productUrls,
                function (productId, productUrl) {
    ...
    _configureElement: function (element) {
      ...
      if (
        typeof this.options.customAttributes[this.simpleProduct] !== "undefined"
      ) {
        $.each(
          this.options.customAttributes[this.simpleProduct],
        ...
        require(["jqueryHistory"], function () {
          if (
            config.replaceUrl &&
            typeof config.productUrls[simpleId] !== "undefined"
          ) {
      ...
      if (element.value) {
        ...
        if (element.nextSetting) {
          ...
          if (nextSelectedVal.length) {
            if (
              $("#" + nextId + " option[value='" + nextSelectedVal + "']")
                .length
            ) {
          ...
          if (!!document.documentMode) {
            ...
            this.inputSimpleProduct.val(
              element.options[element.selectedIndex].config.allowedProducts[0]
            );
          } else {
            this.inputSimpleProduct.val(
              element.selectedOptions[0].config.allowedProducts[0]
            );
          }
        }
      } else {
    ...
    _fillSelect: function (element) {
      ...
      if (options) {
        if (typeof this.options.preSelectedOption !== "undefined") {
          ...
          $.each(
            preSelectedOption,
            $.proxy(function (preattributeId, preoptionId) {
        ...
        for (i = 0; i < options.length; i++) {
          ...
          if (typeof this.options.preSelectedOption !== "undefined") {
            ...
            if (
              this.options.isExtensionEnable != 0 &&
              this.options.isFirstSelected == 0
            ) {
    ...
    _getPrices: function () {
      ...
      _.each(
        ...
        function (element) {
          ...
          if (
            config &&
            config.allowedProducts.length === 1 &&
            !hasProductPrice
          ) {
    ...
    _calculatePrice: function (config) {
      var displayPrices = $(this.options.priceHolderSelector).priceBox(
        ...
        newPrices =
          this.options.spConfig.optionPrices[_.first(config.allowedProducts)];
